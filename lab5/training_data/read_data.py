...
datadir_test='test_data'    # directory where the data files reside
...
def read_data_test():
    ...
    Rows=64    # all images are 64x64
    Cols=64
    n=len(dataset_test)*len(datachar)  # total number of images
    p=Rows*Cols   # number of pixels

    X=np.zeros((p,n))  # images arranged in columns of X
    k=0
    for dset in dataset_test:
        for ch in datachar:
            fname='/'.join([datadir_test,dset,ch])+'.tif'
            im=Image.open(fname)
            img = np.array(im)
            X[:,k]=np.reshape(img,(1,p))
            k+=1
